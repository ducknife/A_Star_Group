...
export function DocumentCard({ document }: { document: AppDocument }) {
  ...
  const title = pickLang(language, document.title, document.titleEn);
  const description = pickLangOptional(language, document.description, document.descriptionEn);
  ...
  return (
    <Card className="flex h-full flex-col">
      <div className="aspect-[4/3] w-full overflow-hidden border-b border-ink-200 bg-ink-100 dark:border-ink-800 dark:bg-ink-800">
        <img src={document.thumbnailUrl} alt={title} className="h-full w-full object-cover" />
      </div>

      <div className="flex flex-1 flex-col p-5">
        <Badge className="self-start">{t.categories[document.category]}</Badge>
